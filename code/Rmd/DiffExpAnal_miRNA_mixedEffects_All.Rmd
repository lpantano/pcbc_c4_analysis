...
```{r voom-norm}
voom_mat = voom(mat_sub, design = model.matrix(~ 0 + ., model.frame(~ . , meta_sub[,new_factors])), plot=FALSE)
...
meta_sub$Cell_Type_of_Origin_Level2  = gsub("[^[:alnum:]]","",meta_sub$Cell_Type_of_Origin_Level2)
...
```{r limma-heatmap}
keep = unique(diffname$GeneSymbol)
...
CODE <- synStore(CODE, used = c(mat_obj$properties$id,metadata_obj@schema),activityName=activityName, executed=thisFile)
...
DE_FC <- synStore(DE_FC, used = c(mat_obj$properties$id,metadata_obj@schema),activityName=activityName, executed=CODE)
...
DE_PV <- synStore(DE_FC, used = c(mat_obj$properties$id,metadata_obj@schema),activityName=activityName, executed=CODE)
...
SIG.SETS_OBJ <- File('../../data/DiffExpAnal_miRNA_mixedEffects_All_diffmiRNAs.tsv',name = 'Differentily Expressed miRNA comparison list',parentId = CODE$properties$id)
...
rlog <- synStore(rlog, used = c(mat_obj$properties$id,metadata_obj@schema),activityName='miRNA voom expression', executed=CODE)
