...
.list = lapply(states, function(state){
  ...
  .genes=lapply(unique(mrna_mirna_res$contrast)[idx], function(comp){
    ...
    if (.side==3){
      ...
      .res_meth = meth_mrna_res %>% filter(contrast==comp & gene_fc < 0 & meth_fc > 0)
    }else{
      .res = mrna_mirna_res %>% filter(contrast==comp & mrna_fc > 0)
      .res_meth = meth_mrna_res %>% filter(contrast==comp & gene_fc > 0 & meth_fc < 0)
    }
    return(list(mirna=as.character(.res$gene), meth=as.character(.res_meth$ensembl)))
    # return(list(mirna=as.character(.res$gene), meth=c()))
  })
  
  
  
  .genes_mirna = unique(unlist(sapply(.genes, function(x){x$mirna})))
  .genes_meth = unique(unlist(sapply(.genes, function(x){x$meth})))
  ...
  return(list(entrez=.entrez, ensembl=.all, mirna=.mirna, meth=.meth, genes_mirna=.genes_mirna))  
...
all_info = lapply(as.character(levels(.states_lvs)), function(this_state){
  ...
  print(colSums(.toy=="Target")/nrow(.toy))
