...
all_info = lapply(as.character(levels(.states_lvs)), function(this_state){
  ...
  .top_mirna = as.character(unique(unlist(.top_mirna$mirna[1:10])))
...
# Color dark if miRNA is top in the list of that state 
...
gmirna = ggplot(.ma_melt, aes(x=variable, y=state, fill=value)) +
  ...
  theme_minimal() + theme(plot.margin = unit(c(-1,-0.8,0,0), "lines"),
...
gmrna = ggplot(.mrna_melt, aes(y=gene, x=variable, fill=value)) +
  ...
  theme_minimal() + theme(plot.margin = unit(c(0,0.8,-0.5,-0.5), "cm"),
                          panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
...
axis_df = data.frame(x=1:7, y =1:7, text=colnames(.ma_mrna))
axisPlot <- ggplot(axis_df, aes(x=x,y=y, fill="white")) +
  geom_tile()+
  geom_text(aes(label = text), size=3) +
  scale_fill_manual(guide=FALSE, values = "white") + 
  theme(
    plot.margin = unit(c(-0.8,0.4,4.1,-0.5), "cm"),
