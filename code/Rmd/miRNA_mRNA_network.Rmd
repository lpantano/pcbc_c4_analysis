...
.list = lapply(states, function(state){
  ...
  .genes_mirna_counts = data.frame(genes=unlist(sapply(.genes, function(x){x$mirna}))) %>% group_by(genes) %>% summarise(total=n())
  .genes_meth_counts = data.frame(genes=unlist(sapply(.genes, function(x){x$meth}))) %>% group_by(genes) %>% summarise(total=n())
...
cols = colorRampPalette(rev(c("red","orange","blue")))(100)
