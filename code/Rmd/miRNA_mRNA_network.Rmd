...
only taking interactions that appear  > 1 times

```{r network-gene-mirna-no-meso}
dd_reduce = merge(dd, dd_times, by="id") %>% filter( times>5) %>% dplyr::arrange(desc(times))
con = rbind( data.frame(start=dd_reduce$mirna, end=dd_reduce$gene),
             data.frame(start=dd_reduce$contrast, end=dd_reduce$mirna ))
con = con %>% distinct()
suppressMessages(library(igraph))
...
```{r c3-network}
meth_mirna_res = do.call(rbind,network_meth_list)
mrna_mirna_res = do.call(rbind,network_list)

c3_res = merge(mrna_mirna_res, meth_mirna_res, by ='mirna') %>% filter(contrast.x==contrast.y)
```
