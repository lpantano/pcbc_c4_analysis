...
.list = lapply(states, function(state){
  ...
  .genes=lapply(unique(mrna_mirna_res$contrast)[idx], function(comp){
    ...
    return(list(mirna=unique(as.character(.res$gene)), meth=as.character(.res_meth$ensembl)))
    # return(list(mirna=as.character(.res$gene), meth=c()))
  })
...
all_info = lapply(as.character(levels(.states_lvs)), function(this_state){
  ...
  # .all_genes = as.character(unique(c(.all_genes_mi, .all_genes_meth)))
  ...
  .toy = matrix(ncol=nrow(.mirna_exp), nrow=length(.mirna_genes))
  rownames(.toy)= .mirna_genes
  colnames(.toy)= rownames(.mirna_exp)
  .toy[is.na(.toy)] = 0
  .void = apply(.filtered_mi, 1, function(x){
    x = as.character(x)
    if ( sum(x[7]==rownames(.toy))>0 & sum(x[1]==colnames(.toy))>0 )
      .toy[x[7], x[1]] <<- .toy[x[7], x[1]] + 1
  })
  
  print(colSums(.toy==1)/nrow(.toy))
  .mirna_mrna_ma = .get_mirna_mrna_ma(.toy,min_total = 0.1)  
  ...
  .ann_df=as.data.frame(.toy_red[,.toy_hc$order])

  .regulated = intersect(rownames(.toyb_red)[rowSums(.toyb_red)>0], rownames(mrna_keep))
  
  idx = match(.list[[this_state]]$ensembl, meth_mrna_res$ensembl)
  .meth = meth_mrna_res$symbol[idx[!is.na(idx)]]
  .in_meth = rownames(mrna_keep)[rownames(mrna_keep) %in% .meth]
  # .regulated_meth = setdiff(intersect(.in_meth, rownames(mrna_keep)), .regulated)
  .regulated_meth = intersect(.in_meth, rownames(mrna_keep))
  ...
  if (length(.regulated_meth) > 0){
    # print(.regulated_meth)
    .ma_meth = mrna_keep[.regulated_meth, ]
    .ma_hc_meth = hclust(as.dist(1-cor(t(.ma_meth))^2))
    .all_ma = rbind(.ma[.ma_hc$order ,.states_lvs], .ma_meth[.ma_hc_meth$order ,.states_lvs])
  }else{
  ...
  .all_ann[.in_meth, "methylated"] = "Target"
  ...
  list(ma = .all_ma, ann=.all_ann, col=.ann_colors, 
       used=.mirna_mrna_ma$used, meth_used = .regulated_meth, not_explained=.unkown)
})
```
...
ann_df = as.data.frame(.toy_red[,.toy_hc$order])
.all_meth = do.call(rbind, lapply(all_info, function(x){x$ann[,"methylated", drop=FALSE]}))
ann_df$methylated = "None"
ann_df[row.names(.all_meth)[!is.na(.all_meth$methylated)], "methylated"] = "Target"
#cbind(.ann_df, all_meth[rownames(.ann_df),])
# .all_ann[.in_meth, "methylated"] = "None"
# .all_ann[, "methylated"] = "Target"
...
.void = lapply(all_info, function(x){
  ...
  for (.m in unique(.seen)){
    # print(.m)
    # print(table(.ann_df[rownames(x$ma),.m]))
...
ann_colors = lapply(ann_colors,function(x){
  c(x, Top10="black")
})
...
rownames(mirna_exp) = names(ann_df)[1:27]
...
pheatmap(.ma, color = cols, show_colnames = TRUE, cluster_cols = FALSE, cluster_rows = FALSE, scale='none', annotation_legend = FALSE)

.ma_melt =  melt(.ma %>% mutate(state=rownames(.ma)))
.ma_melt$state = factor(.ma_melt$state, levels(.states_lvs))
.ma_melt$variable = factor(.ma_melt$variable, colnames(.ma))

mirna_map = ggplot(.ma_melt, aes(x=variable, y=state, fill=value)) +
  geom_tile() + 
  scale_fill_gradient2(guide=FALSE, low="darkorange3", high="deepskyblue4", mid = "lightyellow", midpoint = 0) +
  theme_minimal() +
...
write.table(all_ma, "../../data/miRNA-mRNA_mixedEffect_median_exp_by_group.tsv", quote=F, sep="\t", row.names=F)
res <- File('../../data/miRNA-mRNA_mixedEffect_median_exp_by_group.tsv',name = 'gene median expression by stage',parentId = parentId)
res <- synStore(res, used = ALL_USED_IDs, activityName=ActivityName, executed=CODE)

write.table(mrna_mirna_res, "../../data/miRNA-mRNA_mixedEffect_gene_annotation.tsv", quote=F, sep="\t", row.names=F)
