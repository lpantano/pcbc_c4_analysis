...
nothing here, there is not interaction involving miRNA-methylation-miRNA
...
ma_plot = dd_reduce %>% separate(contrast, c("type", "void", "g1", "vs", "g2"), sep = "_", remove=TRUE) %>%
  dplyr::select(id, mirna, gene, mirna_fc, mrna_fc, g1, g2)

suppressMessages(library(igraph))

.convert_to_gene <- function(m){
  require(biomaRt)
  ensembl = useMart('ensembl', dataset = "hsapiens_gene_ensembl")
  df = getBM(attributes=c("ensembl_gene_id", "external_gene_name"), filters="ensembl_gene_id", values=as.character(m), mart=ensembl)
  return(df)
}
...
for (u_id in unique(ma_plot$id)){
  ...
  gr<-graph.data.frame(con[,c("g1","g2", "mrna_fc")], directed = T)
  V(gr)$color<-"white"
  plot.igraph(gr, layout=layout.auto,edge.color="grey80",vertex.label.cex=0.7,vertex.size=35,edge.width=5,vertex.label.color="black", main=paste(con$mirna[1], "->" ,name_gene[2]))
}
