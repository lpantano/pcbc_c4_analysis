...
```{r}
  .ann_colors_b = lapply(colnames(.ann_df), function(x) {c("azure", "grey")})
  names(.ann_colors_b) = colnames(.ann_df)
  
  .ann_colors_b[['methylated']] = c(None="white", Target="grey")
...
.void = lapply(all_info, function(x){
  ...
  for (.m in unique(.seen)){
    ...
    # print(sum(.ma))
    if (sum(.ma)>0)
      .ann_df[rownames(x$ma),.m][.ma] <<- "Detected"
  }
